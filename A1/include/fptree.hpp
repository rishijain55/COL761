//todo: add minimum len to function of fptree_growth_time
#ifndef FPTREE_HPP
#define FPTREE_HPP
...
std::pair<std::set<Pattern>,bool> fptree_growth(const FPTree& fptree){
    ...
    if(std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() > lim){
        return {{},false};
    }
    else{
        return {st,true};
...
std::set<Pattern> fptree_growth_time(const FPTree& fptree, Timepoint Tim, int lim) // bound the time of running algo
{
    ...
    if(std::chrono::duration_cast<std::chrono::microseconds>(curr - Tim).count() > lim){
        return {};
    }
